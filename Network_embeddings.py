...
class EndToEndNetwork:
    def __init__(self, num_hops=1, batch_size=30, validation_size=500, sentence_input_embed_dim=500, num_variants = 2
                 ):
        ...
        self.sentence_ouput_embed_dim = sentence_input_embed_dim
        self.vocabluary_size = len(self.batch_generator.encode_dict)
        self.memory_dim = self.sentence_output_embed_dim
    ...
    def build_graph(self):
        ...
        for j in range(self.num_hops):
            sentences_output_embed = tf.matmul(
                tf.reshape(
                    batch_sentences,
                    [self.batch_size * self.num_variants, -1]),
                C[j]) + bc

            sentences_input_embed = tf.matmul(
                tf.reshape(
                    batch_sentences,
                    [self.batch_size * self.num_variants, -1]),
                A[j]) + ba
        ...
        #
        # cross_entropy = cross_entropy / tf.constant(float(self.batch_size))
        ...
        self.train_step = tf.train.AdamOptimizer(learning_rate=0.0001).minimize(cross_entropy)
 ...
 #           batch = self.batch
    ...
    def validate(self, print_examples, print_score=True, verbose = True):
        validation_set = self.batch_generator.get_sum_batch(batch_size=self.validation_size, batch_type='validation')
...
def train_yahoo():
    n = EndToEndNetwork(num_variants=20)
    n.build_graph()
    n.init_variables()
    n.train(1000)
